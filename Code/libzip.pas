...
function zip_open(zipname: pansichar; level: integer = ZIP_RDONLY; plevel: pinteger = nil): Pzip_t; cdecl; external zipdll; overload;
function zip_open(zipname: ansistring; level: integer = ZIP_RDONLY; plevel: pinteger = nil): Pzip_t; overload;
...
function zip_stat(z: Pzip_t; const Name: pansichar; flags: integer; var st: Zip_Stat_t): integer; cdecl; external zipdll;
function zip_fopen(zip: Pzip_t; entryname: pansichar; f: integer = ZIP_FL_UNCHANGED): Pzip_file_t; cdecl; external zipdll;
...
function zip_name_locate(zip: Pzip_t; const fname: pansichar; flags: integer = 1): int64; cdecl; external zipdll;
function zip_has_file(zip: Pzip_t; fname: ansistring): boolean;
...
function zip_file_add(archive: Pzip_t; const Name: pansichar; Source: Pzip_source_t; flags: integer): uint64; cdecl; external zipdll;
...
function zip_getsize(zip: Pzip_t; filename: pansichar): longint;
...
function zip_open(zipname: ansistring; level: integer = ZIP_RDONLY; plevel: pinteger = nil): Pzip_t; overload;
begin
  result := zip_open(pansichar(zipname), level, plevel);
end;

function zip_has_file(zip: Pzip_t; fname: ansistring): boolean;
begin
  result := zip_name_locate(zip, pansichar(fname)) >= 0;
end;
...
begin
  ...
  try
    zip_stat_init(zs);
    if zip_stat(zip, pansichar(filename), ZIP_FL_UNCHANGED, zs) <> 0 then
      Exit();
